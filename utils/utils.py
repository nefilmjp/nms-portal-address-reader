...
def normalize_and_grayscale(img: cv2.Mat):
...
def get_glyph_from_address(
    img: cv2.Mat, pos: int, resolution: dict[str, Union[int, float]]
):
    """pos: 0-11"""
    top = 0
...
def parse_standard(path: str):
    ...
    for pos in range(16):
        ...
        sample = cv2.resize(glyph, (320, 320), None, None, None, cv2.INTER_NEAREST)
...
def match_address(img: cv2.Mat, std_kps, std_descs, resolution):
    ...
    for pos in range(12):
        glyphColor = get_glyph_from_address(address, pos, resolution)
        glyphGray = normalize_and_grayscale(glyphColor)

        sample = cv2.resize(glyphGray, (320, 320), None, None, None, cv2.INTER_NEAREST)
...
def get_thumb_ss(ss: cv2.Mat, resolution):
    img = crop_address(ss, resolution)

    thumbs = []

    for pos in range(12):
        glyphColor = get_glyph_from_address(img, pos, resolution)
        glyphGray = normalize_and_grayscale(glyphColor)
        thumbs.append(glyphGray)
